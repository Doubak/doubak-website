...
- **换底色要重算对比度。** 现在是浅绿底 + 深绿字，都取自站点那套变量。实测
  （顶部 / 底部，底色有渐变所以两头不一样）：名字 `#0f3d20` 10.92 / 9.55，标语
  `#1d5c33` 7.09 / 6.21，最下一行 `#2f6b47` 5.63 / **4.92**（小字要 4.5，底部
  最紧的地方也还有富余），分隔线 `#2e963d` 3.36 / 2.94（纯装饰，不承载信息）。
  **底色往深里走会先压垮最下面那行。**
- **标记和名字必须同色。** 它们是一个整体（lockup）。给标记单独上品牌绿试过，看着
  更花，而且浅底上品牌绿只有 3.36:1，缩小之后「豆」字先糊掉。品牌绿只留在分隔线上。
