...
| `gemini/` | 第一版草稿，**没有采用**，留作来历 | — |

`gemini/` 里是最早那一版：`promo-raw.jpeg` 是用 Gemini 生成的大图，
`promo-440x280-full.png` 与 `promo-440x280-tight.png` 是从它裁出来的两个 440×280
版本（一个保留全图、一个收紧留白）。**都不是现在在用的那张**——现在用的是上面那份
SVG。留着是因为它是这张图的来历，不是备选方案；要改设计改 SVG，别从这里翻出一张来传。
...
- **换底色要重算对比度。** 底是 `#eef6f0`（green-soft），**平铺，没有渐变**，
  所以对比度只有一个数，比渐变好算。标记、名字、标语、分隔线全是 logo 那支绿
  `#2e963d`，实测 3.44:1——图形要 3.0 够，40px 的名字和 25px 的标语算大字也要
  3.0，够。**只有最下面那行 14.5px 的小字要 4.5，logo 绿撑不住**，所以换成
  `#007722`（green-link，5.21:1）。
  换句话说，logo 绿在这张图上唯一撑不住的地方就是字号最小的那一行。全部统一成
  logo 绿也做得出来，两版都渲染出来比过——代价是那行字在缩略图里明显先糊。
...
JPEG 用 `jpegoptim --strip-all`（同样无损，只是重排 Huffman 表并去掉元数据）。
**去元数据这一步别省**：`promo-raw.jpeg` 原来带着一段 `app11`，而这个仓库里的东西
是要发到公网上的。
