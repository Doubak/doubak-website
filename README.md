...
robots.txt            points crawlers at the sitemap, and lists what is not a page
...
- **What stays out of the sitemap is decided by `robots.txt`, not by the workflow.**
  The action reads the `Disallow:` rules under `User-agent: *`, so the two files that
  are not pages — `404.html` and `log/_template.html` — are listed once, in the one
  file that also tells real crawlers about them.
- A page carrying `<meta name="robots" content="noindex">` is dropped too. Use that
  when a page should stay crawlable but unlisted; use `robots.txt` when it should not
  be fetched at all.
